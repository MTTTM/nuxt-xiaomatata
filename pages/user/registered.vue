    ...
    <b-form @submit="onSubmit" class="px-3 py-3">
      <b-overlay :show="pushing" rounded="sm">
        ...
        <b-form-group label="邀请码:">
          <b-form-input
            v-model="$v.form.invite_code.$model"
            :state="validateState('invite_code')"
            type="password"
            placeholder="请输入邀请码"
          ></b-form-input>
          <div
            class="error"
            v-if="
              !validateState('invite_code') &&
              validateState('invite_code') !== null
            "
          >
            请输入长度为4的数字或字母的邀请码
          </div>
        ...
        <NuxtLink to="/user/signin">有账号，去登录</NuxtLink>
      </b-overlay>
    </b-form>
    <template #footer>
